    ...
    <header className="flex h-12 shrink-0 items-center gap-3 border-b border-zinc-200 bg-white px-4">
          ...
          >
            ...
            <kbd
              className={`rounded px-1 text-[9px] font-semibold ${
                mode === m ? "bg-white/20 text-white" : "bg-zinc-200 text-zinc-500"
              }`}
            >
              {KEYS[m]}
            </kbd>
      ...
      <span
        className={`hidden items-center gap-1 text-[10px] sm:flex ${
          snapHint ? "text-indigo-600" : "text-zinc-400"
        }`}
        title="Hold Shift while dragging to snap"
      >
        ⇧ Shift = snap {snapHint ? "on" : ""}
      </span>

      <div className="ml-auto flex items-center gap-1.5">
        {status && (
          <span className="mr-1 hidden text-[11px] text-emerald-600 md:inline">{status}</span>
        )}
        <button
          type="button"
          onClick={onSave}
          className="rounded-md border border-zinc-300 bg-white px-2.5 py-1 text-xs text-zinc-700 transition hover:bg-zinc-100"
        >
          💾 Save
        </button>
        <button
